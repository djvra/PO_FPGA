...
#define ROWS 3
#define COLUMNS 3
#define FILE_PATH "input/data3.txt"
...
void write_point_to_file_decimal(int result_point[][ROWS], int point_count) {
    char filename[256];
    sprintf(filename, "%d_all", COLUMNS);
    ...
    for(int j = 0; j < point_count; j++){
        for (int i = 0; i < rows_m1; i++) {
            fprintf(fp, "%" PRIu8 " ", result_point[j][i]);  
        }
        fprintf(fp, "%" PRIu8 "\n", result_point[j][rows_m1]);
    }
...
void dump_result_point_array(int result_point[][ROWS], int point_count) {
    char filename[256];
    sprintf(filename, "C_result_point_dump_%d_yeni", COLUMNS);
...
void check_40bit_signed_range(__int128_t num) {
    ...
    if (num > MAX_39_BIT_SIGNED || num < MIN_39_BIT_SIGNED) {
        printf("The number ");
        print_int128(num);
        printf(" is outside the 39-bit signed integer range.\n");
    } 
}
...
void isLargerThanUint8(int number) {
    if (number > 255) {
        printf("The number %d is larger than 255.\n", number);
    }
}
...
int main(void)
{
    ...
    long long int fi[ROWS] = {0};
    long long int si[ROWS] = {0};
       ...
       int temp_index = Winv_index;
        for(int i = 0 ; i < ROWS ; i++){
            for(int j = 0 ; j < COLUMNS ; j++) {
                ...
                //check_40bit_signed_range(input_array[temp_index] * q_hat[j]);
                check_long_long_range(q_hat[j]);
                ...
                //check_40bit_signed_range(q_trans[i]);
        ...
        for (int i = 0 ; i < ROWS ; ++i) {
            printf("q_int[%d] = ", i);
            print_int128(q_int[i]);
            printf("\n");

            check_40bit_signed_range(q_int[i]);
        }
        ...
        while (1)
        {
            ...
            for(int i = 0 ; i < ROWS ; i++){
                ...
                for(int j = 0 ; j < COLUMNS ; j++){
                    ...
                    check_40bit_signed_range(input_array[Winv_index + i * ROWS + j] * v_inner[j]);

                    inner_Res[i]  = inner_Res[i]  + input_array[Winv_index + i * ROWS + j] * v_inner[j];

                    check_40bit_signed_range(inner_Res[i]);
                ...
                if(inner_Res[i]  < 0) {
                    /*printf("inner_Res[%d]: ", i);
                    print_int128(inner_Res[i]);
                    printf(" + last_diagonal: ", i);
                    print_int128(last_diagonal);
                    printf(" = ");
                    print_int128(inner_Res[i] + last_diagonal);
                    printf("\n");*/
            ...
            for(int i = 0 ; i < ROWS ; i++){
                __int128_t outer = 0;
                for(int j = 0 ; j < COLUMNS ; j++){
                    //printf("V_index + i * ROWS + j: %d\n", V_index + i * ROWS + j);
                    /*printf("inner_Res[j]: ");
                    print_int128(inner_Res[j]);
                    printf("\n");*/
                    /*printf("outer = ");
                    print_int128(outer);
                    printf("\n");*/

                    check_40bit_signed_range(input_array[V_index + i * ROWS + j] * inner_Res[j]);
                    ...
                    check_40bit_signed_range(outer);
                }
                ...
                if (outer < 0) {
                    printf("outer is less than 0\n");
                }

                //long long int test_var_2 = outer + q_summand[i];
                //check_int_range(test_var_2);
                //check_40bit_signed_range(test_var_2);
                //check_short_int_range(test_var_2);

                check_40bit_signed_range(outer + q_summand[i]);

                result_point[point_count][i] = (outer + q_summand[i]) / last_diagonal;

                isLargerThanUint8(result_point[point_count][i]);

                printf("%u ", result_point[point_count][i]);
           ...
           if(point_count == 1000){
            ...
            int i = ROWS-1;
            while(i >= 0) {
                if (v_inner[i] > max_v_inner) {
                    max_v_inner = v_inner[i];
                }
    ...
    for (int i = 0; i < ROWS; ++i) {
        printf("first if: %d => %lld\n", i, fi[i]);
    }

    for (int i = 0; i < ROWS; ++i) {
        printf("second if: %d => %lld\n", i, si[i]);
    }
